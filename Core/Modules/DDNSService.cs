    ...
    public sealed class DDNSService
    {
        ...
        public async Task UpdateAsync()
        {
            try
            {
                ...
                var responseMessage = await client.GetAsync("https://www.ip.cn/api/index?ip=&type=0");
                if (responseMessage.StatusCode != System.Net.HttpStatusCode.OK) throw new Exception("获取外网IP失败");
                var result = (await responseMessage.Content.ReadAsStringAsync()).FromJson<GetIpResult>();
                if (result == null || string.IsNullOrEmpty(result.Ip)) throw new Exception("获取外网IP失败");
                var original = await _resolver.UpdateRecordAsync(_config.Domain, _config.SubDomain, result.Ip);
                if (original != result.Ip) _logger.LogInformation($"域名解析更新成功：{original} => {result.Ip}");
                else _logger.LogDebug($"域名解析未更新：{result.Ip}");
            }
            catch (Exception ex)
            {
